...
claude, codex, opencode, pi, pi-signed, grok, kimi, and cursor are verified for crewmate and secondmate launches; [README requirements](../README.md#requirements) own the set supported for the primary session.
...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, OpenCode uses its TUI plugin, and Cursor uses native hooks plus managed background-task completion.
Cursor workers use the ACP bridge rather than TUI parsing; [`cursor.md`](cursor.md) owns the complete primary/worker contract, model and effort behavior, and verification gates.
...
Cursor accepts the model axis but has no separate verified effort axis, so a non-default Cursor effort is refused before endpoint creation instead of being silently omitted.
