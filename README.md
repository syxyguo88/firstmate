...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, or Cursor CLI.
...
Codex, OpenCode, and Cursor are also verified and supported as primary harnesses.
Codex uses bounded foreground checkpoints, OpenCode uses a TUI plugin, and Cursor uses native hooks plus managed background-task completion, so each carries harness-specific supervision tradeoffs.
Cursor crewmates and secondmates use the official ACP protocol instead of TUI parsing; see [Cursor harness support](docs/cursor.md).
...
For Cursor, authenticate the `agent` CLI first; the tracked `.cursor/hooks.json` and `.cursor/skills` integration loads from the repository root.
...
- [docs/cursor.md](docs/cursor.md) - Cursor primary hooks, ACP crewmates and secondmates, model/effort behavior, and live verification gates.
